...
The algorithm guaranteed to work eventually, no matter the difficulty, but the solving time is somewhat up to chance. In the vast majority of cases the solution will be found in under 15 seconds but due to the fixed order in which the program proceeds it is theoretically possible the algorithm may take much longer than that.
-------
